...
//    //Write a JPQL query to list all admin accounts
    @Query(" select account from AccountDetail account where account.role='ADMIN_USER'")
...
//
    ...
    @Query(value = "select * from account_details where name ilike concat('%',?1,'%') ?1 or address ilike concat('%',?1,'%') or country ilike concat('%',?1,'%') or state ilike concat('%',?1,'%') or cityilike concat('%',?1,'%')", nativeQuery = true)
